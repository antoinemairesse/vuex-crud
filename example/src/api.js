...
mock.onPost("/books").reply((config) => {
  return [204, Object.assign(structuredClone(getRandom(fakeData.books)), JSON.parse(config.data))]
});
mock.onPost("/books/filter").reply(200, {
  books: fakeData.books,
});
mock.onPut("/books").reply((config) => {
  return [200, Object.assign(structuredClone(getRandom(fakeData.books)), JSON.parse(config.data))]
});
